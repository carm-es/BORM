...
public class ParseadorXMLCades extends ParseadorXML {
	...
	public ParseadorXMLCades(String xml)
	...
	public boolean pendienteDeActualizacion() {
		...
		if (resultado.equals(ParseadorXMLCades.PENDIENTE)) {
			return true;
		}
		return false;
	}
	...
	public boolean hayError() {
		String resultado = getCodigoResultado();
		if (resultado.equals(ParseadorXMLCades.ERROR_TIPO_1) || resultado.equals(ParseadorXMLCades.ERROR_TIPO_2)) {
			return true;
		}
		return false;
	}
	...
	public boolean hayExito() {
		String resultado = getCodigoResultado();
		if (resultado.equals(ParseadorXMLCades.EXITO)) {
			return true;
		}
		return false;
	}
	...
	public byte[] getFirmaActualizada() throws IOException {
		Node raiz = doc.getDocumentElement();
		Node optionalOutputs = getNode("dss:OptionalOutputs", raiz.getChildNodes()); 
		Node documentWithSignature = getNode("dss:UpdatedSignature", optionalOutputs.getChildNodes());
		Node document = getNode("dss:SignatureObject", documentWithSignature.getChildNodes());			
		Node base64XML = getNode("dss:Base64Signature", document.getChildNodes());
